---
title: "Day 5 (Security) - Mentor's Page"
permalink: "/day5/mentors_bWVudG9y"
---

# Notes
* Day 1 of the workshop taught `while` loops; `for` loops can be interchangeably in this activity too
* `0b11111111` is the same as `0xFF`


# Solutions

## Extract the secret image with `n` significant bits
`pixel << (8-n) & 0b11111111` 

## Remove the the last three bits of the medium pixel
`pixel >> 3 << 3`
...
## Move the `n` most significant bits of the secret image into the right spot
`secret >> 8-n`

## Combine the two components for hiding
`a | b`

# Solution Code
```python3
# To extract: (img << (8-n)) & 0xFF
# To hide: (medium >> n << n) | secret >> (8 - n)
